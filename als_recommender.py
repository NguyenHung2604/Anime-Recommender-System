...
class ExplicitALSRecommender:
    ...
    def find_anime_id_by_name(self, anime_df: pd.DataFrame, anime_name: str) -> int:
        ...
        exact_match = anime_lookup[anime_lookup["name"].astype(str).str.lower() == anime_name.lower()]
        if not exact_match.empty:
            return int(exact_match.iloc[0]["anime_id"])

        partial_match = anime_lookup[anime_lookup["name"].astype(str).str.contains(anime_name, case=False, na=False)]
    ...
    def similar_anime(
        ...
        similarity = self.item_factors @ query_vector
        similarity = similarity / (norms * query_norm + 1e-12)
        similarity = similarity.astype(np.float32)
        similarity[query_idx] = -np.inf
...
def main() -> None:
    ...
    if args.anime_id is not None or args.anime_name is not None:
        ...
        print(f"Top {args.top_k} anime tương tự với: {query_name} (anime_id={query_anime_id})")
